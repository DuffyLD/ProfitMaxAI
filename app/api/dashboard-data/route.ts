...
export async function GET(req: Request) {
  try {
    ...
    // ---- top sellers (within window) + titles via latest snapshot ----------
    const topSellers = (await sql/*sql*/`
      ...
      select
        ...
        ls.product_title::text as product_title,
        ls.variant_title::text as variant_title
      from order_items oi
      join orders o on o.id = oi.order_id
      left join last_snap ls on ls.variant_id = oi.variant_id
      where o.shop_domain = ${shop}
        and o.created_at >= now() - make_interval(days => ${windowDays})
      group by oi.variant_id, ls.product_title, ls.variant_title
      order by qty_sold desc, oi.variant_id asc
      limit 10;
    `) as any[];

    // ---- slow-movers query -------------------------------------------------
    // latest snapshot per variant (WITH titles) + sales in window + last sold (all-time)
    ...
    return NextResponse.json({
      ok: true,
      shop,
      // helps you confirm which deployment is live
      build: {
